  ...
  return `<svg viewBox="0 0 160 240" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;overflow:visible">
    <defs>
      <linearGradient id="outfitGrad" x1="0.15" y1="0" x2="0.85" y2="1">
        <stop offset="0%" stop-color="${tint(b,0.3)}"/>
        <stop offset="22%" stop-color="${tint(b,0.1)}"/>
        <stop offset="55%" stop-color="${b}"/>
        <stop offset="100%" stop-color="${darken(b,32)}"/>
      </linearGradient>
      <linearGradient id="pantsGrad" x1="0.2" y1="0" x2="0.8" y2="1">
        <stop offset="0%" stop-color="${tint(p,0.24)}"/>
    ...
    <g opacity=".5" style="mix-blend-mode:screen">
      <path d="M92 134 Q94 155 93 176" fill="none" stroke="${tint(b,0.26)}" stroke-width="5" stroke-linecap="round"/>
      <path d="M110 140 Q112 158 109 176" fill="none" stroke="${tint(b,0.26)}" stroke-width="6" stroke-linecap="round"/>
    </g>
