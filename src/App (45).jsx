...
>
  ...
  const charAnimBlock = (size, height) => {
    const isLarge = parseInt(size) >= 200;
    ...
    const w = size; const h = height || (isLarge ? "250px" : "72px");
  ...
  return (
    <div style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      ...
      {isMobile ? (
        ...
        // ── Cinematic frame, not sidebar + chat ──────────────────────────────
        // The character is composed off-axis and large, bleeding past the bottom edge
        // like a film still rather than sitting centered in a bounded avatar box. The
        // room's light is the mood signal (color/spread driven by presenceMood), not a
        // separate badge. The transcript is a borderless ribbon on the opposite side,
        // masked to fade into the scene instead of being clipped by a hard panel edge.
        const MOOD_GLOW = { warm:"#C8A870", amused:"#C8A870", surprised:"#A8B8D0", thinking:"#7A8AA0", skeptical:"#8A7A60", serious:"#4A4040", uncomfortable:"#8A6060", listening:"#8AA08A", neutral:charMeta.accent };
        const glow = MOOD_GLOW[presenceMood] || charMeta.accent;
        return (
        <div style={{ flex:1, position:"relative", overflow:"hidden", background:isTransitioning?"#080808":`radial-gradient(ellipse 90% 75% at 78% 96%, ${charMeta.bg} 0%, #0A0A0A 55%, #060606 100%)`, transition:"background .7s ease" }}>
          {/* Directional mood light — the actual emotional signal, replacing the status badge */}
          <div style={{ position:"absolute", inset:0, pointerEvents:"none", background:`radial-gradient(circle 480px at 82% 100%, ${glow}30 0%, transparent 70%)`, transition:"background 1.1s ease", zIndex:1 }}/>

          {/* Character — large, off-center, bleeding off the bottom edge */}
          <div style={{ position:"absolute", right:"6%", bottom:"-6%", width:"420px", height:"560px", zIndex:2, pointerEvents:"none", opacity:isTransitioning?0:1, transition:"opacity .5s ease" }}>
            {charAnimBlock("420px", "560px")}
          </div>

          {/* Name/title — sits at the character's feet, not above in a centered label block */}
          <div style={{ position:"absolute", right:"8%", bottom:"6%", zIndex:3, textAlign:"right", pointerEvents:"none" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"24px", color:charMeta.accent }}>{charMeta.name}</div>
            <div style={{ fontSize:"9px", color:"#C8A458", letterSpacing:".14em", textTransform:"uppercase", marginTop:"4px" }}>{charMeta.title}</div>
            {isSpeaking && <div style={{ display:"flex", gap:"3px", alignItems:"flex-end", justifyContent:"flex-end", marginTop:"10px" }}>{[0,.1,.2,.1,0].map((d,i)=><div key={i} style={{ width:"2px", height:`${5+i*3.5}px`, background:charMeta.accent, borderRadius:"1px", animation:`waveBar .6s ease-in-out ${d}s infinite`, opacity:.6 }}/>)}</div>}
          </div>

          {/* Transcript ribbon — borderless, fades into the scene top/bottom, no card */}
          <div style={{ position:"absolute", top:0, left:0, bottom:0, width:"min(46%, 560px)", display:"flex", flexDirection:"column", zIndex:4 }}>
            <div style={{ flex:1, overflowY:"auto", padding:"40px 36px 24px 48px", display:"flex", flexDirection:"column", gap:"28px", position:"relative", WebkitMaskImage:"linear-gradient(to bottom, transparent 0, black 48px, black calc(100% - 48px), transparent 100%)", maskImage:"linear-gradient(to bottom, transparent 0, black 48px, black calc(100% - 48px), transparent 100%)" }}>
              <div style={warmOverlayStyle(warmMode, { position:"30% 20%", fade:"75%", transitionMs:1000 })}/>
              {msgList("15px")}
              {loading && loadingDots()}
              {error && <p style={{ fontSize:"11px", color:"#BC5A5A", textAlign:"center" }}>{error}</p>}
              <div ref={chatEndRef}/>
            </div>
            <div style={{ padding:"0 36px 28px 48px" }}>
              {actionRow()}
