  ...
  return (
    <div style={{ ...BASE, display:"flex", flexDirection:"column" }}>
      ...
      {isMobile ? (
        ...
        // One continuous lit space, not two boxed panels: the character's scene-light
        // gradient spans the full stage and the chat surface floats over it, so presence
        // and conversation read as the same room instead of a sidebar bolted to a chat app.
        <div style={{ flex:1, display:"flex", overflow:"hidden", position:"relative", background:isTransitioning?"#080808":`radial-gradient(ellipse 70% 60% at 16% 38%, ${charMeta.bg} 0%, #080808 62%)`, transition:"background .6s ease" }}>
          {/* Character — fixed presence anchor, no card/border, lives in the same light as the chat */}
          <div style={{ width:"320px", flexShrink:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", position:"fixed", top:"56px", left:0, height:"calc(100vh - 56px)", overflow:"hidden", zIndex:10, pointerEvents:"none" }}>
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"18px", padding:"32px 16px 24px", width:"100%" }}>
              {charAnimBlock("220px")}
              <div style={{ textAlign:"center", opacity:isTransitioning?0:1, transition:"opacity .38s", width:"100%" }}>
                <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:"19px", color:charMeta.accent, marginBottom:"6px" }}>{charMeta.name}</div>
          ...
          {/* Desktop chat — floats over the scene, no dividing border against the character */}
          <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", marginLeft:"320px", position:"relative" }}>
            <div style={{ flex:1, overflowY:"auto", padding:"32px 40px 16px", display:"flex", flexDirection:"column", gap:"28px", alignItems:"stretch", position:"relative" }}>
              <div style={warmOverlayStyle(warmMode, { position:"50% 15%", fade:"75%", transitionMs:1000 })}/>
              <div style={{ maxWidth:"640px", width:"100%", margin:"0 auto", display:"flex", flexDirection:"column", gap:"28px" }}>
                {msgList("15px")}
                {loading && loadingDots()}
                {error && <p style={{ fontSize:"11px", color:"#BC5A5A", textAlign:"center" }}>{error}</p>}
              </div>
